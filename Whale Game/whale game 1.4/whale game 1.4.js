...
const whale = {
    ...
    draw: function() {
        ...
        // Highlight (top stripe, precise path)
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.3)'; // Lighter highlight stroke
        ctx.lineWidth = 4; // Thickness of the highlight
        ctx.beginPath();
        ctx.moveTo(noseX - bodyWidth * 0.2, centerY - bodyHeight * 0.7); // Start slightly behind nose
        ctx.quadraticCurveTo(
            centerX + bodyWidth * 0.05, centerY - bodyHeight * 1, // Curve apex
            centerX - bodyWidth * 0.6, centerY - bodyHeight * 0.6  // End near mid-body top
        );
        ctx.stroke();
...
const SPARK_MAX_AGE = 15; // Shorter life for sparks
...
function createFlare() {
    ...
    flares.push({
        ...
        scored: false,
        trail: [],
        sparks: [] // Array to hold spark particles
    });
}
...
// --- NEW: DRAW FLARE SPARKS ---
function drawFlareSparks(f) {
    ctx.globalCompositeOperation = 'lighter'; // Makes sparks glow
    f.sparks.forEach(s => {
        const opacity = 1 - (s.age / SPARK_MAX_AGE);
        ctx.fillStyle = `rgba(255, 120, 0, ${opacity})`; // Orange glow for sparks
        ctx.beginPath();
...
function drawFlares() {
    ...
    flares.forEach(f => {
        ...
        // Update screen positions for the trail and sparks
        ...
        drawFlareSparks(f); // Draw sparks on top of the trail

        // --- 1. Main Flare Body (Darker, Smoldering appearance) ---
        ctx.globalAlpha = 1.0;
        ctx.fillStyle = 'rgba(100, 30, 0, 0.8)'; // Dark, smoldering core color
        ctx.beginPath();
        ...
        // --- 2. Inner Glow (Subtle Reddish-Orange) ---
        ctx.globalCompositeOperation = 'lighter';
        ctx.fillStyle = `rgba(255, 100, 0, 0.5)`; // Soft orange inner glow
        ctx.beginPath();
...
function updateFlares() {
    ...
    for (let i = flares.length - 1; i >= 0; i--) {
        ...
        // 3. Update Sparks
        if (frameCount % 1 === 0) { // Add new sparks frequently
            f.sparks.push({
                worldX: f.worldX + f.width / 2 + (Math.random() - 0.5) * f.width,
                worldY: f.worldY + f.height * 0.3 + (Math.random() - 0.5) * f.height * 0.6,
                radius: Math.random() * 2 + 1,
                age: 0,
                speedX: (Math.random() - 0.5) * 1.5, // Horizontal drift
                speedY: (Math.random() - 1) * 2 // Rise and fall slightly
            });
        }
