// Game constants and state
        ...
        const totalScoreDisplay = document.getElementById('total-score-display'); // <-- NEW
        ...
        function generateBubbles() {
            ...
            for(let i = 0; i < 300; i++) { 
                // --- PARALLAX EFFECT ---
                // 0.2 = far away, 0.8 = close
                const parallax = Math.random() * 0.6 + 0.2; 
                // Scale radius and opacity based on parallax
                const scale = parallax / 0.8; 

                bubbles.push({
                    worldX: WORLD_LEFT + Math.random() * bubbleAreaWidth, 
                    worldY: surfaceWorldY + Math.random() * bubbleAreaHeight, 
                    radius: (Math.random() * 4 + 1) * scale, // Far bubbles are smaller
                    opacity: (Math.random() * 0.3 + 0.05) * scale, // Far bubbles are fainter
                    speedY: (Math.random() * 0.5 + 0.1) * scale, // Far bubbles rise slower
                    life: Math.random() * 100,
                    parallax: parallax // Store parallax factor
                });
            }
        }
        
        function drawBubbles() {
            // No need for viewport vars here, calculated per-bubble
        ...
        function drawSeabed() {
            // --- PARALLAX SEABED ---
            const parallax = 0.5; // Seabed is in the mid-ground
            ...
            if (seabedScreenY < canvas.height) {
                ...
                // 2. Draw simple coral/rock structures (also parallaxed)
                ...
                structures.forEach(s => {
                    ...
                    // Scale structures by parallax factor
        ...
        function drawFlares() {
            ...
            flares.forEach(f => {
                ...
                // --- NON-PARALLAXED ---
                // Flares are "in the same plane" as the whale, so they don't get parallax.
                // We just need their screen positions relative to the camera.
                const flareScreenX = f.worldX - viewportWorldX;
                const flareScreenY = f.worldY - viewportWorldY;

                // Update trail/spark screen positions
                f.trail.forEach(p => { 
        ...
        function startGame() {
            ...
            generateBubbles(); // Generate new parallax bubbles
        ...
        function handleCollision() { 
            ...
            setTimeout(() => {
                ...
                // --- NEW: Add current score to total score ---
                let currentTotal = parseInt(localStorage.getItem('whaleDefenderTotalScore') || 0, 10);
                currentTotal += score; // Add this game's score
                localStorage.setItem('whaleDefenderTotalScore', currentTotal);
                totalScoreDisplay.textContent = `Total Points: ${currentTotal}`;
                // --- END NEW ---
        ...
        window.onload = function() {
            ...
            messageBox.classList.add('visible');
            generateBubbles(); // Generate initial set
            whale.draw();

            // --- NEW: Load total score and add reset logic ---
            let currentTotal = parseInt(localStorage.getItem('whaleDefenderTotalScore') || 0, 10);
            totalScoreDisplay.textContent = `Total Points: ${currentTotal}`;

            document.getElementById('reset-total-score').addEventListener('click', (e) => {
                e.stopPropagation(); // Stop click from starting game
                // Use a custom modal for confirmation instead of confirm()
                if (document.getElementById('confirm-reset-modal')) return; // Don't open if already open
                ...
                confirmModal.style.backgroundColor = 'rgba(0, 0, 0, 0.95)';
                ...
                confirmModal.innerHTML = `
                    <p style="margin-top: 0;">Are you sure you want to reset your total points to 0?</p>
                    <button id="confirm-yes" style="margin-right: 10px;">Yes, Reset</button>
                    <button id="confirm-no" style="background-color: var(--sea-blue-mid);">No</button>
                `;
                ...
                document.getElementById('confirm-yes').addEventListener('click', () => {
                    ...
                    totalScoreDisplay.textContent = 'Total Points: 0';
