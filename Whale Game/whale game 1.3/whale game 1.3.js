...
const waveAmplitude = 18; // Slightly bigger waves
const waveLength = 80;
let waveOffset = 0; 
...
const WORLD_DEPTH = 1500; // Deeper world
...
const whale = {
    ...
    draw: function() {
        ...
        ctx.bezierCurveTo(
            centerX - bodyWidth * 0.9, centerY + bodyHeight * 0.8, 
            centerX + bodyWidth * 0.4, centerY + bodyHeight * 1.2, 
            noseX, centerY // Back to Nose tip
        );
        ...
        ctx.lineTo(tailX, centerY); // Close shape
        ctx.fill();

        // Highlight (top stripe, slightly behind the nose)
        ctx.fillStyle = 'rgba(255, 255, 255, 0.3)'; // Lighter highlight
        ctx.beginPath();
        ctx.ellipse(centerX, centerY - bodyHeight * 0.5, bodyWidth * 1.2, bodyHeight * 0.2, 0, 0, Math.PI);
        ctx.fill();
...
function generateBubbles() {
    ...
    for(let i = 0; i < 300; i++) { 
        bubbles.push({
            ...
            radius: Math.random() * 4 + 1, // Smaller and more varied
            opacity: Math.random() * 0.3 + 0.05, // More subtle
            speedY: Math.random() * 0.5 + 0.1, // Gives them individual rising speed
...
function drawBubbles() {
    ...
    bubbles.forEach(b => {
        b.worldY -= b.speedY * 0.1; // Slow constant rise

        // Loop bubbles back to the bottom when they hit the top
...
function drawCaustics() {
    ...
    for (let i = 0; i < 6; i++) { // More caustics bands
        ctx.beginPath();
        const startX = (i * 150 + frameCount * 2.5) % (canvas.width * 3) - canvas.width * 1.5; // Slower, wider movement

        for (let x = 0; x < canvas.width; x += 15) {
            const noise = Math.sin((x * 0.03) + frameCount * 0.005) * 50; // Smoother noise
...
function drawDepthHaze() {
    // Creates a subtle darkening and blurring effect the deeper the whale is
    ...
    // Haze opacity increases from 0 to 0.4 as whale dives deeper
...
function drawSky() {
    ...
    if (waveScreenY > 0) {
        ...
        ctx.strokeStyle = '#6d9aa3'; // Slightly darker line
        ctx.lineWidth = 2;
        ctx.beginPath();
...
function drawWaves() {
    ...
    for (let x = 0; x <= canvas.width; x += 10) {
        ...
        // Add a small second sine wave for complexity
...
const TRAIL_MAX_AGE = 40; // Longer trail
...
function drawFlareTrail(f) {
    f.trail.forEach(p => {
        ...
        // Draw as a slightly bubbly/smoky white
...
function drawFlares() {
    ...
    flares.forEach(f => {
        ...
        // --- 1. Flare Body & Red Core ---
        ctx.globalAlpha = 1.0;

        // Draw the main body (pyramid shape) - Dark Red/Orange
        ctx.fillStyle = getCssVar('--ui-accent'); 
        ctx.beginPath();
        ...
        // --- 2. Inner Fire Glow (Lighter Composite) ---
        ctx.globalCompositeOperation = 'lighter';
        
        // Pulsating Orange/Yellow flame shape
        const flameHeight = f.height * 0.6 + Math.sin(frameCount * 0.2) * 5; 
        ctx.fillStyle = `rgba(255, 153, 0, 0.9)`; 
        ctx.beginPath();
        ctx.ellipse(screenX + f.width / 2, screenY + f.height * 0.3, f.width / 3, flameHeight / 2, 0, 0, Math.PI * 2);
        ctx.fill();

        // Bright White Center
        ctx.fillStyle = `rgba(255, 255, 200, 0.9)`;
        ctx.beginPath();
        ctx.arc(screenX + f.width/2, screenY + f.height * 0.1, 5, 0, Math.PI * 2); 
        ctx.fill();
...
function updateFlares() {
    ...
    for (let i = flares.length - 1; i >= 0; i--) {
        ...
        // 2. Update Trail (more random trail generation)
        if (frameCount % 2 === 0) { 
            f.trail.push({
                worldX: f.worldX + f.width / 2 + (Math.random() - 0.5) * 8, // Wider spread
                worldY: f.worldY + f.height,
                radius: Math.random() * 4 + 1.5, // Larger max radius
                age: 0
            });
        }
        ...
        if (screenY + f.height > whaleHitAreaTop && screenY < whaleHitAreaBottom) {
            ...
            // Horizontal overlap (using the whale's full body width)
...
function gameLoop() {
    ...
    // 1. Clear & Background Layers (Crucial Order)
    ...
    // 3. Final Layer: Depth Haze (Must be last to cover everything)
    drawDepthHaze();
...
document.addEventListener('keydown', (e) => {
    ...
    if (isGameOver && (e.code === 'Space' || e.code === 'Enter') && messageBox.classList.contains('visible')) {
        // Ensure starting only happens if the modal is visible
